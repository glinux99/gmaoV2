import { Tt as j, bt as c, dt as W, it as R, nt as P } from "./dist-ChbKImyH.js";
import { t as BaseStyle } from "./style-VofgM0UA.js";
import { t as BaseDirective } from "./basedirective-BaHqCGMd.js";
//#region node_modules/primevue/styleclass/style/index.mjs
var StyleClassStyle = BaseStyle.extend({ name: "styleclass-directive" });
//#endregion
//#region node_modules/primevue/styleclass/index.mjs
var StyleClass = BaseDirective.extend({ style: StyleClassStyle }).extend("styleclass", {
	...
	methods: {
		bind: function bind(el, binding) {
			...
			el.$_pstyleclass_clicklistener = function() {
				if (binding.value.toggleClass) if (R(target, binding.value.toggleClass)) P(target, binding.value.toggleClass);
				else W(target, binding.value.toggleClass);
				else if (target.offsetParent === null) _this.enter(target, el, binding);
		...
		enter: function enter(target, el, binding) {
			if (binding.value.enterActiveClass) {
				if (!target.$_pstyleclass_enter_animating) {
					...
					if (binding.value.enterFromClass) P(target, binding.value.enterFromClass);
					target.$p_styleclass_enterlistener = function() {
						P(target, binding.value.enterActiveClass);
						if (binding.value.enterToClass) W(target, binding.value.enterToClass);
						target.removeEventListener("animationend", target.$p_styleclass_enterlistener);
						target.removeEventListener("animationcancel", target.$p_styleclass_entercancellistener);
						if (binding.value.enterActiveClass.includes("slidedown")) target.style.maxHeight = "";
			...
			if (binding.value.hideOnOutsideClick) this.bindDocumentListener(target, el, binding);
			if (binding.value.hideOnResize) this.bindResizeListener(target, el, binding);
		},
		leave: function leave(target, binding) {
			if (binding.value.leaveActiveClass) {
				if (!target.$_pstyleclass_leave_animating) {
					...
					if (binding.value.leaveFromClass) P(target, binding.value.leaveFromClass);
					target.$p_styleclass_leavelistener = function() {
						P(target, binding.value.leaveActiveClass);
						if (binding.value.leaveToClass) W(target, binding.value.leaveToClass);
			...
			if (binding.value.hideOnOutsideClick) this.unbindDocumentListener(target);
			if (binding.value.hideOnResize) this.unbindResizeListener(target);
		},
		bindDocumentListener: function bindDocumentListener(target, el, binding) {
			...
			if (!target.$p_styleclass_documentlistener) {
				target.$p_styleclass_documentlistener = function(event) {
					if (!_this2.isVisible(target) || getComputedStyle(target).getPropertyValue("position") === "static") _this2.unbindDocumentListener(target);
					else if (_this2.isOutsideClick(event, target, el)) _this2.leave(target, binding);
				};
		...
		bindResizeListener: function bindResizeListener(target, el, binding) {
			...
			if (c(target.$p_styleclass_resizeselector)) this.bindElementResizeListener(target, binding);
			else this.bindWindowResizeListener(target, binding);
		},
		...
		bindWindowResizeListener: function bindWindowResizeListener(target, binding) {
			...
			if (!target.$p_styleclass_windowresizelistener) {
				target.$p_styleclass_windowresizelistener = function() {
					if (!_this3.isVisible(target)) _this3.unbindWindowResizeListener(target);
					else _this3.leave(target, binding);
				};
		...
		bindElementResizeListener: function bindElementResizeListener(target, binding) {
			...
			if (!target.$p_styleclass_resizeobserver && target.$p_styleclass_resizeselector) {
				...
				target.$p_styleclass_resizeobserver = new ResizeObserver(function() {
					...
					if (_this4.isVisible(target)) _this4.leave(target, binding);
				});
