...
$backlogHours = $backlogTasksQuery->sum('estimated_duration') +
    ...
    'mttr' => round($mttr, 1),
    'mtbf' => round($mtbf, 1),
    'preventiveComplianceRate' => round($preventiveComplianceRate),
    'backlogTasksCount' => $backlogCount,
    'availabilityRate' => round(max(0, $availabilityRate * 100), 1),
    'oee' => round(max(0, min(100, $oee)), 1),
    'backlogHours' => round($backlogHours / 60, 1),

    // Graphiques
    'sparePartsMovement' => $sparePartsMovement,
    'stockRotationData' => $stockRotationData,
    ...
    'maintenanceStatusDurationChart' => $maintenanceStatusDurationChart,
    'topFailingEquipmentsChart' => $topFailingEquipmentsChart,
    'maintenanceCostDistribution' => $maintenanceCostDistribution,
    ...
    // Données pour les filtres (Listes de sélection)
    'equipments' => Equipment::select('id', 'designation')->get(),
    'zones' => Zone::select('id', 'title')->get(),
]);
    }
